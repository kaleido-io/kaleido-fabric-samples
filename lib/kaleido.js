...
class KaleidoClient {
  ...
  async init() {
    ...
    this.config = await this.buildNetworkConfig(consortium, environment, memberships.map(m => m._id));
  }
  ...
  async buildNetworkConfig(consortiumId, envId, memberships) {
    ...
    for (let membership of memberships) {
      const {url: ordererUrl, id: ordererId} = await this.locateOrderer(consortiumId, envId, membership);
      const {url: peerUrl, id: peerId} = await this.locatePeer(consortiumId, envId, membership);
      config.organizations[membership] = {
        mspid: membership,
        peers: [ peerId ],
        orderers: [ ordererId ],
        certificateAuthorities: [this.cas[membership].id]
      };
      config.peers[peerId] = {
        url: peerUrl,
        tlsCACerts: {
      ...
      config.certificateAuthorities[this.cas[membership].id] = {
        url: this.cas[membership].url,
        caName: "",
        tlsCACerts: {
          pem: [caCertPEM]
        },
  ...
  async locateOrderer(consortiumId, envId, membershipId) {
    ...
    if (result.data.length === 0) {
      ...
      if (orderersForMembership.length === 0) {
        console.error(`No orderers found for the membership ${membershipId}`);
        process.exit(1);
