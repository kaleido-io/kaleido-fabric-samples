...
class KaleidoClient {
  constructor(userId) {
    ...
    this.kaleidoUrl =
      process.env.KALEIDO_URL || "https://console.kaleido.io/api/v1";
  }
  ...
  async init() {
    ...
    this.channel = await this.locateChannel(
      consortium,
      environment,
      memberships
    );
    ...
    if (!user) {
      ...
      user = await this.wallet.newUser(
        this.userId,
        secret,
        this.cas[this.myMembership].url
      );
    }
    this.user = user;

    this.config = await this.buildNetworkConfig(
      consortium,
      environment,
      memberships
    );
  ...
  async locateMembership(consortiumId) {
    let result = await axios.get(
      `${this.kaleidoUrl}/c/${consortiumId}/m`,
      this.apiAuth
    );
    let membership;
    if (result.data.length === 0) {
      console.error(
        `No memberships found in the business network ${consortiumId}`
      );
      process.exit(1);
    } else if (result.data.length > 1) {
  ...
  async locateEnvironment(consortiumId) {
    let result = await axios.get(
      `${this.kaleidoUrl}/c/${consortiumId}/e`,
      this.apiAuth
    );
    let env;
    if (result.data.length === 0) {
      console.error(
        `No environments found in the business network ${consortiumId}`
      );
      process.exit(1);
    } else if (result.data.length > 1) {
  ...
  async locateChannel(consortiumId, environmentId) {
    let result = await axios.get(
      `${this.kaleidoUrl}/c/${consortiumId}/e/${environmentId}/channels`,
      this.apiAuth
    );
    let ret;
    if (result.data.length === 0) {
      console.info(
        `No channels found in the environment ${environmentId}. The "default-channel" should have been created. Something went wrong in the environment. Exiting.`
      );
      process.exit(1);
    } else if (result.data.length > 1) {
  ...
  async locateOrderers(consortiumId, envId) {
    ...
    if (result.data.length === 0) {
      ...
      this.orderers = orderers.map((orderer) => {
        ...
        const { orgCA: caCertPEM } = JSON.parse(
          Buffer.from(orderer.node_identity_data, "hex").toString()
        );
        return {
  ...
  async locatePeers(consortiumId, envId) {
    ...
    if (result.data.length === 0) {
      ...
      this.peers = peers.map((peer) => {
        ...
        const { orgCA: caCertPEM } = JSON.parse(
          Buffer.from(peer.node_identity_data, "hex").toString()
        );
        return {
  ...
  async getCertFile(membershipId) {
    ...
    const caId =
      this.config.organizations[membershipId].certificateAuthorities[0];
  ...
  async buildNetworkConfig(consortiumId, envId, memberships) {
    ...
    for (let membership of this.channel.members) {
      const orderers = this.orderers.filter(
        (orderer) => orderer.membershipId === membership
      );
      config.channels[this.channel.name].orderers = config.channels[
        this.channel.name
      ].orderers.concat(orderers.map((o) => o.id));

      const peers = this.peers.filter(
        (peer) => peer.membershipId === membership
      );
      config.channels[this.channel.name].peers = config.channels[
        this.channel.name
      ].peers.concat(peers.map((p) => p.id));
    }
    for (let membership of memberships) {
      ...
      config.organizations[membershipId] = {
        ...
        orderers: this.orderers
          .filter((o) => o.membershipId === membershipId)
          .map((o) => o.id),
        certificateAuthorities: this.cas[membershipId]
          ? [this.cas[membershipId].id]
          : [],
      };
    }
